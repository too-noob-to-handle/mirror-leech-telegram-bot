...
aria2c --allow-overwrite=true --auto-file-renaming=true --bt-enable-lpd=true --bt-detach-seed-only=true \
       ...
       --max-connection-per-server=16 --max-concurrent-downloads=10 --max-file-not-found=0 --max-tries=20 \
       --min-split-size=8M --optimize-concurrent-downloads=true --peer-id-prefix=-qB4390- --reuse-uri=true \
       --peer-agent=qBittorrent/4.3.9 --quiet=true --rpc-max-request-size=1024M --seed-ratio=0 --split=16 \
